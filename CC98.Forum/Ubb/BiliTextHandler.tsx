...
export default class BiliTextHandler extends Ubb.UbbTextHandler {
  ...
  exec(match: RegExpMatchArray, context: Ubb.UbbCodeContext): React.ReactNode {
    ...
    //判断是否是av号
    const isAv = /^(https?:\/\/)?www\.bilibili\.com\/video\/av[0-9]+/.test(
      content
    );
    //判断是否是BV号
    const isBV =
      /^(https?:\/\/)?www\.bilibili\.com\/video\/BV[0-9A-Za-z]+/.test(content);
    //获取url中分P数
