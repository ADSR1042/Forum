...
export class UbbTagSegment extends UbbSegment {
   ...
   */
  private static forceClose(
    segment: UbbSegment,
    newParent: UbbTagSegment
  ): void {
    // 文字标签无需关闭
    if (segment.type === UbbSegmentType.Text) {
      ...
      if (seg.isClosed) {
        ...
        console.warn(
          "标签 %s 没有正确关闭，已经被转换为纯文字。",
          seg.tagData.tagName
        );

        // 未关闭标签，自己将被转换为纯文字
        newParent._subSegments.push(
          new UbbTextSegment(seg.tagData.startTagString, segment.parent)
        );
  ...
  getFullText(): string {
    return (
      this.tagData.startTagString +
      this.getContentText() +
      this.tagData.endTagString
    );
  }
}
      ...
      function scanToken(lastTokenType: TokenType): Token {
        ...
        while (true) {
          ...
          if (/\s/i.test(c)) {
            ...
            const matchExp =
              lastTokenType === TokenType.ItemSeperator ? /[=,]/i : /,/i;

            // 寻找下个分隔符
            const nextSeperator = tokenString
              .substring(index + 1)
              .match(matchExp);
            ...
            if (nextSeperator) {
              ...
              return Token.stringValue(
                tokenString.substring(start, endMarkLocation)
              );
            } else {
              // 找不到下一个符号，说明这是最后一个符号
...
export abstract class UbbTagHandler {
   ...
   * @param content 标签的内容。
   */
  protected static renderTagAsString(
    tagData: UbbTagData,
    content: ReactNode
  ): ReactNode {
...
export abstract class TextTagHandler extends UbbTagHandler {
  exec(tagSegment: UbbTagSegment, context: UbbCodeContext) {
    return this.execCore(
      tagSegment.getContentText(),
      tagSegment.tagData,
      context
    );
  }
...
class UbbHandlerList {
   ...
   */
  private registerNamedCore(
    tagNames: string[],
    tagHandler: UbbTagHandler
  ): void {
...
export class UbbCodeEngine {
   ...
   */
  private static tryHandleEndTag(
    tagName: string,
    ...
    // 没有找到任何上级
    console.warn(
      "UBB: 找不到结束标签 %s 的开始标签，该标签将被作为一般文字处理。",
      tagName
    );
      ...
      if (endTagMatch) {
        ...
        try {
          ...
          if (!handler) {
            ...
            parent.subSegments.push(
              new UbbTextSegment(tagData.startTagString, parent)
            );
            continue;
          }
          ...
          switch (tagMode) {
            ...
            case UbbTagMode.Text:
              const endTagMatch = content.indexOf(
                tagData.endTagString,
                tagMatchRegExp.lastIndex
              );
              // 没找到结束标签
              if (endTagMatch === -1) {
                console.warn(
                ...
                // 让下次标签搜索直接从之后开始
                tagMatchRegExp.lastIndex =
                  endTagMatch + tagData.endTagString.length;
              }
              break;
            // 单模式标签
            ...
            default:
              throw new Error(
                `UBB: 标签 ${tagData.tagName} 的处理程序发生错误`
              );
          }
        } catch (error) {
          // 提取数据失败，则视为没有匹配
          console.warn(
            "UBB: 标签字符串 %s 解析失败，将被视为普通文字。",
            tagString
          );
