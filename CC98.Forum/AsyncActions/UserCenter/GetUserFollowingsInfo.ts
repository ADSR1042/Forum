 ...
 * @author AsukaSong
 */
export const getUserFollowingsInfo: ActionCreator<
...
> = (page: number) => async (dispatch, getState) => {
  try {
    ...
    let shouldLoad =
      store.currentUserFollowingInfo.length < (page - 1) * 10 + 1;
    // 当用户向前翻页，且之前的页数中存在未加载过的项
    if (!shouldLoad) {
      // 需要加载就不用判断了，节省资源
      for (
        let i = (page - 1) * 10;
        i < Math.min(page * 10, store.currentUserFollowingInfo.length);
        i++
      ) {
    ...
    followData.forEach(
      (item, index) => (prevFollow[index + (page - 1) * 10] = item)
    );
