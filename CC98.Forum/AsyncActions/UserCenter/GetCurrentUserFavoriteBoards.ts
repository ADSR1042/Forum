 ...
 * @author AsukaSong
 */
export const getCurrentUserFavoriteBoards: ActionCreator<
  ThunkAction<Promise<Action>, RootState, null, RootAction>
> = () => async (dispatch, getState) => {
  try {
    dispatch(Actions.userCenterLoading());
    const currentBoardIds = getState().userInfo.currentUserInfo.customBoards;
    const prevBoardIds = getState().userInfo.currentUserFavoriteBoards.map(
      (item) => item.id
    );
    // 如果用户信息中的版面IDs和store中的信息不匹配则需要重新加载
    const shouldLoading =
      currentBoardIds.length !== prevBoardIds.length ||
