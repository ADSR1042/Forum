...
export async function getUsersInfo(
  ...
  try {
    ...
    if (shouldUseIndexedDb) {
      infos = (await getIndexedDBUsersInfo(keys)).map(
        (item, index) => item || keys[index]
      );
    } else {
      infos = keys.map(
        (item) =>
          getLocalStorage(
            typeof item === "number" ? `userId_${item}` : `userName_${item}`
          ) || item
      );
    }

    // 批量查询未命中的项
    let querys = infos.filter(
      (item) => typeof item === "number" || typeof item === "string"
    ) as number[] | string[];
    let queryInfo: UserInfo[] = [];
    if (querys.length !== 0) {
      const url = keys.some((key) => typeof key === "number")
        ? `/user?id=${querys.join("&id=")}`
        : `/user/name?name=${querys.map(encodeURIComponent).join("&name=")}`;
