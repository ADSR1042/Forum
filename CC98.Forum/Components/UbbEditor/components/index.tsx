...
export class UbbEditor extends React.PureComponent<Props, State> {
  ...
  private changeExtend(extendTagName: string) {
    ...
    this.setState((prevState: State) =>
      extendTagName === prevState.extendTagName
        ? {
            extendTagName: "",
          }
        : {
  ...
  private selectTextArea() {
    ...
    this.customTextArea.textarea.setSelectionRange(
  ...
  render() {
    return (
      <div
        className="ubb-editor"
        style={{ height: `${this.props.option.height || 20}rem` }}
      >
        <Buttons
        ...
        <Extends
          ...
          extendIsShown={
            this.state.extendTagName && this.state.extendTagName !== "emoji"
          }
          shouldCompassImage={this.state.shouldCompassImage}
          changeShouldCompassImage={this.changeShouldCompassImage}
        />
        {this.state.isPreviewing ? (
          <div className="ubb-preview">
            <UbbContainer code={this.state.value} />
          </div>
        ) : (
          <CustomTextArea
            ...
            onPaste={(e) => {
              ...
              const kinds = Array.from(e.clipboardData.items).map(
                (item) => item.kind
              );
