...
export function getNewState(
  state: UbbEditorStateInfo,
  ubbSegment: ConfigType.IUbbSegment
): UbbEditorStateInfo {
...
export function getTagName(file: File) {
  try {
    ...
    switch (true) {
      case config.fileExtendedNames.audio.indexOf(fileExtendName) !== -1:
        return "audio";
      case config.fileExtendedNames.video.indexOf(fileExtendName) !== -1:
        return "video";
      case config.fileExtendedNames.image.indexOf(fileExtendName) !== -1:
        return "img";
      default:
        throw new Error();
    }
