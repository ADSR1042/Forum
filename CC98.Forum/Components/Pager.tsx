  ...
  render() {
    ...
    return (
      <li className="page-item">
        <Link className={`page-link${active ? " active" : ""}`} to={url}>
          {text}
        </Link>
      </li>
    );
    ...
    return (
      <div className="pagination">
        <ul className="page-nav">
          {first && <PageModel text="1" url={`${url}1#1`} />}

          {prev && <PageModel text="&lsaquo;" url={`${url}${curPage - 1}#1`} />}
          ...
          {next && <PageModel text="&rsaquo;" url={`${url}${curPage + 1}#1`} />}

          {last && (
            <PageModel text={`${totalPage}`} url={`${url}${totalPage}#1`} />
          )}
        </ul>
        ...
        {goto && (
          <div className="page-goto">
            <div>
              <input onChange={this.handleInput} placeholder={`跳转到`} />
            </div>
            <div>
