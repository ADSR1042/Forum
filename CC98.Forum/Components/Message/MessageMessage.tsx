...
export class MessageMessage extends React.Component<{}, MessageMessageState> {
  ...
  async componentDidMount() {
    ...
    //如果有未读消息则清空联系人缓存
    if (
      $("#unreadCount-messageCount1").attr("class") === "message-counterNav"
    ) {
    ...
    recentContact = await Utility.sortContactList(
      recentContact,
      this.context.router
    );
  ...
  coverMessagePerson = (item: MessagePersonInfo) => {
    ...
    async function changeChatName() {
      ...
      for (let i in recentContact) {
        if (
          recentContact[i].id === item.id &&
          recentContact[i].isRead === false
        ) {
    ...
    return (
      <div onClick={changeChatName} id={`contact_${item.id}`}>
  ...
  async getMoreContact() {
    if ($("#moreDone").css("display") == "none") {
      ...
      let recentContact = Utility.getStorage("recentContact");
      let newContact = await Utility.getRecentContact(
      ...
      return (
        <div className="message-message">
          <div className="message-message-people">
            ...
            <div className="message-message-pList"></div>
          </div>
        ...
        <div className="message-message">
          ...
          <div className="message-message-people">
            ...
            <div className="message-message-pList">
              ...
              <div
                className="message-message-plMore"
                onClick={this.getMoreContact}
              >
                <img
                  id="moreImg"
                ...
                <div id="moreDone" className="displaynone">
                  小伙伴们都出来了~
                </div>
              </div>
            </div>
