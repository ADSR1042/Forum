...
export default class extends React.Component<
  { userinfo: UserInfo; handleChange: Function },
  State
> {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    let yearsOption = array.map((item) => (
    ...
    let mouthsOption = array.map((item) => (
      <option key={item} value={item}>
        {item}
      </option>
    ));
    mouthsOption.unshift(
      <option key={0} value={0}>
        未选择
      </option>
    );
    ...
    return (
      <div className="user-center-config-others">
        <h2>其他设置</h2>
        ...
        <div className="config-birthday">
          ...
          <select
            id="birthdayYear"
            name="birthdayYear"
            value={this.props.userinfo.birthdayYear}
            onChange={(e) => {
              this.props.handleChange(
                e.target.name,
                Number.parseInt(e.target.value)
          ...
          <select
            id="birthdayMonth"
            name="birthdayMonth"
            value={this.props.userinfo.birthdayMonth}
            disabled={this.props.userinfo.birthdayYear === 0}
            onChange={(e) => {
              this.props.handleChange(
                e.target.name,
                Number.parseInt(e.target.value)
          ...
          <select
            id="birthdayDay"
            name="birthdayDay"
            value={this.props.userinfo.birthdayDay}
            disabled={this.props.userinfo.birthdayYear === 0}
            onChange={(e) => {
              this.props.handleChange(
                e.target.name,
        ...
        {this.state.displayTitles.length > 0 ? (
          <div className="config-gender">
            ...
            <select
              ...
              name="DisplayTitleId"
              value={this.props.userinfo.DisplayTitleId}
              onChange={(e) => {
                this.props.handleChange(
                  e.target.name,
                  Number.parseInt(e.target.value)
            ...
            >
              ...
              {this.props.userinfo.userTitleIds.map((item) => {
                let title = this.state.displayTitles.filter(
                  (title) => title.id === item
                );
                if (title.length > 0) {
                  return (
        ...
        <div className="config-introduction">
          ...
          <div>
            <textarea
              name="Introduction"
              value={this.props.userinfo.Introduction}
              maxLength={100}
              onChange={(e) => {
                this.props.handleChange(e.target.name, e.target.value);
              }}
