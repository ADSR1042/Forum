...
export default class extends React.Component<
  UserCenterMyFavoritesBoardProps,
  UserCenterMyFavoritesBoardState
> {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    return (
      <div className="user-center-myfavorite-board">
        <Link
          to={`/board/${this.props.UserFavoritesBoard.id}`}
          title={this.props.UserFavoritesBoard.name}
        >
          <img
            src={`/static/images/_${this.props.UserFavoritesBoard.name}.png`}
          />
        </Link>
        <div className="user-center-myfavorite-board-info">
          <Link
            ...
            title={this.props.UserFavoritesBoard.name}
          >
            <h2>{this.props.UserFavoritesBoard.name}</h2>
          </Link>
          <p>
            版主：{this.props.UserFavoritesBoard.boardMasters.join(" ")}{" "}
            今日主题 {this.props.UserFavoritesBoard.todayCount} / 总主题{" "}
            {this.props.UserFavoritesBoard.topicCount}
          </p>
        </div>
        <button
          type="button"
          className={this.state.isFollowing ? "" : "user-follow-board"}
