...
class Wealth extends React.Component<Props, State> {
  ...
  handleSubmit(e) {
    ...
    try {
      ...
      if (this.state.wealth.match(/[^\d\.]/) || wealth < 10)
        throw new Error("请输入合法的金额");
      ...
      //判断余额
      if (this.props.userInfo.wealth < wealth * userNames.length)
        throw new Error("财富值不足");
      //转账
  ...
  componentWillReceiveProps(nextProps: Props) {
    ...
    if (!nextProps.isLoading && this.props.isLoading) {
      this.setState({
        info: `成功给${nextProps.successNames.join(" ")}转账了总计${
          Number.parseFloat(this.state.wealth) * nextProps.successNames.length
        }个财富值`,
      });
    }
    //同理判断是否出错
        ...
        <form onSubmit={this.handleSubmit}>
          <div className="transfer-wealth-input">
            ...
            <input
              ...
              onChange={(e) =>
                this.handleWealthChange(this.state.wealth, e.target.value)
              }
            />
          </div>
          <div className="transfer-wealth-input">
            ...
            <input
              ...
              onChange={(e) =>
                this.handleWealthChange(e.target.value, this.state.userNames)
              }
            />
          </div>
          <div className="transfer-wealth-input">
          ...
          <div className="transfer-wealth-input">
            ...
            <ul style={{ marginLeft: "2.5rem" }}>
              <li>
                转账需要收取一定的手续费，手续费金额为
                <strong>10% 或者 10 中的较大值</strong>。
              </li>
              <li>
                转账手续费从转账金额中收取，因此对方实际收到的金额会少于您输入的金额。
              </li>
              <li>
