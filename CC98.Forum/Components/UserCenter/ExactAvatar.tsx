...
> {
  ...
  async getPortraitPhotoURL() {
    ...
    let title = this.state.displayTitles.filter(
      (item) => this.props.userInfo.displayTitleId === item.id
    )[0];
    if (title) {
      let imageUrl: string;
        ...
        <div className="user-badge">
          {this.state.displayTitles && this.props.userInfo.userTitleIds
            ? this.state.displayTitles
                .filter(
                  (item) =>
                    this.props.userInfo.userTitleIds.indexOf(item.id) !== -1
                )
                .map((item) =>
            ...
            : null}
          {this.props.userInfo.boardMasterTitles.map((item) =>
            item.boardMasterLevel === 10 ? (
              <p key={item.boardId}>
                {" "}
                {/* 站务主管*/}
                <Link to={`/boardList#${item.boardName}`}>
                  {item.boardName}
                </Link>
