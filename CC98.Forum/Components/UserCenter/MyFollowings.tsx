...
class Following extends React.Component<Props> {
  ...
  render() {
    if (this.props.isLoading) {
      ...
      return (
        <div className="user-center-myfans" style={{ textAlign: "center" }}>
          没有关注
        </div>
      );
    }
    const curPage = parseInt(this.props.match.params.page) || 1;
    //state转换为JSX
    const userFollowings = this.props.userFollowings
      .slice((curPage - 1) * 10, curPage * 10)
      .map((item) => <MyFollowingsUser key={item.id} userFanInfo={item} />);
    //添加分隔线
    ...
    return (
      <div className="user-center-myfans">
        <div className="user-center-myfans-exact">{userFollowings}</div>
        <Pager
          currentPage={curPage}
          totalPage={this.props.totalPage}
          href="/usercenter/myfollowings/"
          hasTotal={true}
        />
      </div>
    );
