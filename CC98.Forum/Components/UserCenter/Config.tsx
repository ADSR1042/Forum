...
class Config extends React.Component<Props, States> {
  constructor(props: Props) {
    ...
    let myInfo: Info = {
      ...
      birthdayYear: props.userInfo.birthday
        ? Number.parseInt(props.userInfo.birthday.slice(0, 4))
        : 0,
      birthdayMonth: props.userInfo.birthday
        ? Number.parseInt(props.userInfo.birthday.slice(5, 7))
        : 0,
      birthdayDay: props.userInfo.birthday
        ? Number.parseInt(props.userInfo.birthday.slice(8, 10))
        : 0,
  ...
  handleReset() {
    ...
    let userInfo: Info = {
      ...
      birthdayYear: info.birthday
        ? Number.parseInt(info.birthday.slice(0, 4))
        : 0,
      birthdayMonth: info.birthday
        ? Number.parseInt(info.birthday.slice(5, 7))
        : 0,
      birthdayDay: info.birthday
        ? Number.parseInt(info.birthday.slice(8, 10))
        : 0,
  ...
  async handleSubmit() {
    ...
    try {
      ...
      //生日年月日分开处理，提交时合并
      newInfo.Birthday =
        this.state.userInfo.birthdayYear !== 0
          ? `${this.state.userInfo.birthdayYear}-${this.state.userInfo.birthdayMonth}-${this.state.userInfo.birthdayDay}`
          : "";

      //检测信息是否正确
      if (
        newInfo.EmailAddress &&
        !newInfo.EmailAddress.match(/[\S]+@[\S]+\.[\S]+/)
      ) {
        throw new Error("请检查邮箱地址");
      }
      if (
        newInfo.QQ &&
        (Number.parseInt(newInfo.QQ) <= 0 ||
          Number.parseInt(newInfo.QQ).toString() !== newInfo.QQ)
      ) {
        throw new Error("请检查QQ是否正确");
      }
      let birthDay = new Date(
        this.state.userInfo.birthdayYear + 10,
        this.state.userInfo.birthdayMonth - 1,
        this.state.userInfo.birthdayDay
      );
      if (
        this.state.userInfo.birthdayYear !== 0 &&
        this.state.userInfo.birthdayYear !== 9999 &&
  ...
  render() {
    return (
      <div className="user-center-config">
        ...
        <hr />
        <ConfigSignature
          signature={this.state.userInfo.SignatureCode}
