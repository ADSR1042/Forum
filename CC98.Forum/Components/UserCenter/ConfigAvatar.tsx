...
class UserCenterConfigAvatar extends React.Component<Props, States> {
        ...
        case "mousemove":
          if (this.dragging !== null) {
            ...
            this.setState((prevState) => {
              ...
              if (y > prevState.naturalHeight - this.state.selectorWidth) {
                y = prevState.naturalHeight - this.state.selectorWidth;
              }
              if (x > prevState.naturalWidth - this.state.selectorWidth) {
                x = prevState.naturalWidth - this.state.selectorWidth;
              }
              return {
                selectorTop: y,
                selectorLeft: x,
      ...
      case "mousemove":
        if (this.dragging !== null) {
          ...
          this.setState((prevState) => {
            ...
            let max = Math.min(
              prevState.NUM_MAX,
              prevState.naturalWidth - prevState.selectorLeft,
              prevState.naturalHeight - prevState.selectorTop
            );
            if (!isNaN(num)) {
              if (num < 100) {
                num = 100;
  ...
  handleSubmit() {
    ...
    ctx.drawImage(
      this.state.img,
      x * this.state.scaling,
      y * this.state.scaling,
      width * this.state.scaling,
      width * this.state.scaling,
      0,
      0,
      width,
    ...
    canvas.toBlob(
      async (result) => {
        ...
        file.name =
          "头像." +
          this.state.fileType.slice(
            this.state.fileType.indexOf("/") + 1,
            this.state.fileType.length
          );
        try {
          //上传头像文件到API
    ...
    return (
      <div>
        ...
        <div className="user-center-config-avatar">
          ...
          <div>
            <button
              id="chooseDefaultAvatar"
              type="button"
              onClick={() =>
                this.setState({ choosingDefault: true, info: "暂时只有六枚" })
              }
            >
              选择论坛头像
            </button>
            <div>
              ...
              <label
                ...
                onClick={() => this.setState({ choosingDefault: false })}
              >
                <p>选择本地图片</p>
              </label>
              ...
              <button
                ...
                style={this.state.isShown ? {} : style}
          ...
          <div
            className="user-center-config-avatar-preview"
            style={
              this.state.isShown
                ? { opacity: 1, marginTop: "2rem" }
                : { zIndex: -1 }
            }
          >
            <div
            ...
            >
              ...
              <div
                ...
                style={
                  this.state.isShown
                    ? {
                        width: `${this.state.selectorWidth}px`,
                        height: `${this.state.selectorWidth}px`,
                        borderRadius: `${this.state.selectorWidth / 2}px`,
                        top: `${this.state.selectorTop}px`,
                        left: `${this.state.selectorLeft}px`,
                      }
                    : style
                }
              >
              ...
              <span
                ...
                style={{
                  top: `${this.state.selectorWidth + this.state.selectorTop}px`,
                  left: `${
                    this.state.selectorWidth + this.state.selectorLeft
                  }px`,
                }}
              ></span>
            </div>
            <img
              onLoad={(e) => {
                this.handleIMGLoad(
                  (e as any).target.naturalWidth,
                  (e as any).target.naturalHeight,
                  (e as any).target
                );
              }}
              style={style}
          ...
          >
            {[
              ...
              <img
                ...
                onClick={this.setDefaultAvatar}
                src={`//www.cc98.org/static/images/${item}`}
              />
            ))}
          </div>
        </div>
