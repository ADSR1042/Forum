...
export class Vote extends React.PureComponent<props> {
  ...
  static isFormIllegal(voteInfo: props["voteInfo"]) {
    if (voteInfo.expiredDays > 1000 || voteInfo.expiredDays < 1)
      return "请输入合法的过期天数";
    if (
      voteInfo.maxVoteCount > voteInfo.voteItems.length ||
      voteInfo.maxVoteCount < 1
    )
      return "请输入合法的最大投票数";
  ...
  addVoteItem() {
    if (this.props.voteInfo.voteItems.length > 19) {
      ...
      this.props.onVoteInfoChange({
        ...this.props.voteInfo,
        voteItems: [...this.props.voteInfo.voteItems, ""],
      });
    }
  }
  ...
  render() {
    ...
    const maxVoteCount =
      voteInfo.maxVoteCount === 0 ? "" : voteInfo.maxVoteCount;
      ...
      <>
        <div className="createTopicTitle">
          ...
          <input
            ...
            onChange={(e) => {
              const newValue = parseInt(e.target.value || "0");
              if (
                newValue > this.props.voteInfo.voteItems.length ||
                newValue < 0
              )
                return null;
              this.changeValue(e.target.name, newValue);
        ...
        {this.props.voteInfo.voteItems.map((item, index) => (
          <div className="createTopicTitle">
            ...
            />
            {this.props.voteInfo.voteItems.length > 2 ? (
              <button
                type="button"
