...
export default withRouter(
  class extends React.Component<
    ...
    { likeNumber; dislikeNumber; likeState }
  > {
    ...
    quote() {
      this.props.quote(
        ...
        this.props.postInfo.time,
        this.props.postInfo.floor,
    ...
    async like() {
      ...
      const status = await Utility.like(
      ...
      switch (status) {
        case 200:
          const data = await Utility.refreshLikeState(
            this.props.topicInfo.id,
            this.props.postInfo.id
          );
          this.setState({
            likeNumber: data.likeCount,
            dislikeNumber: data.dislikeCount,
    ...
    async dislike() {
      ...
      const status = await Utility.dislike(
        this.props.topicInfo.id,
        this.props.postInfo.id,
        this.context.router
      );
    ...
    async componentDidMount() {
      ...
      if (
        ...
        (this.props.boardInfo.id == 144 && this.props.postInfo.isLZ)
      )
    ...
    render() {
      ...
      if (Utility.getLocalStorage("userInfo"))
        if (
          this.isAllowedtoEdit(this.props.userInfo.privilege) ||
          this.props.postInfo.isMe
        ) {
          editIcon = (
            <Link to={editUrl}>
              <div className="operation1" onClick={this.edit}>
                {" "}
                编辑
              </div>
            </Link>
      ...
      let lastUpdate = null;
      if (
        this.props.postInfo.lastUpdateAuthor &&
        this.props.postInfo.lastUpdateTime
      ) {
        const time = moment(this.props.postInfo.lastUpdateTime).format(
          "YYYY-MM-DD HH:mm:ss"
        );
        const name =
          this.props.userInfo.name === this.props.postInfo.lastUpdateAuthor
            ? "作者"
            : this.props.postInfo.lastUpdateAuthor;
      ...
      let traceIcon = (
        <div className="operation1">
          <Link to={this.props.traceMode ? returnUrl : traceUrl}>
            {this.props.traceMode ? "返回" : "追踪"}
          </Link>
        </div>
      );
      ...
      if (
        Utility.isMaster(this.props.boardInfo.boardMasters) == true ||
        (this.props.boardInfo.id === 144 &&
          Utility.getLocalStorage("userInfo") &&
          Utility.getLocalStorage("userInfo").name ===
            this.props.topicInfo.userName)
      ) {
        manageBtn = (
          <div
      ...
      let judgeIcon = (
        <div className="operation1" onClick={this.showJudgeUI}>
          {" "}
          评分
        </div>
      );
      return (
        <div
          className="column"
          style={{ marginTop: "1rem", width: "52rem", marginBottom: "0.5rem" }}
        >
          <div className="comment1">
            ...
            >
              <span>
                发表于{" "}
                {moment(this.props.postInfo.time).format("YYYY-MM-DD HH:mm:ss")}
              </span>
              <span style={{ marginLeft: "1rem" }}>{lastUpdate}</span>
            </div>
            <div className="row" style={{ alignItems: "center" }}>
              <div
                id={idLike}
                className={
                  this.state.likeState === 1 ? "upup red-color" : "upup"
                }
              ...
              >
                <i
                  title="赞"
                  className={
                    this.state.likeState === 1
                      ? "fa fa-thumbs-up fa-lg"
                      : "fa fa-thumbs-o-up fa-lg"
                  }
                ></i>
                <span className="commentProp"> {this.state.likeNumber}</span>
              </div>
              <div
                id={idDislike}
                className={
                  this.state.likeState === 2 ? "downdown red-color" : "downdown"
                }
              ...
              >
                <i
                  title="踩"
                  className={
                    this.state.likeState === 2
                      ? "fa fa-thumbs-down fa-lg"
                      : "fa fa-thumbs-o-down fa-lg"
                  }
                ></i>
                <span className="commentProp"> {this.state.dislikeNumber}</span>
              </div>
              <div id="commentlike">
                {judgeIcon}
                {window.location.pathname.indexOf("postid") === -1 ? (
                  <QuoteContext.Consumer>
                    {(handleQuoteContextChange) => (
                      <div
                        className="operation1"
                        onClick={() => {
                          handleQuoteContextChange({
                            ...
                            replyTime: this.props.postInfo.time,
                            floor: this.props.postInfo.floor,
