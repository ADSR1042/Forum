...
export class VoteContent extends React.PureComponent<props, state> {
  ...
  async submit() {
    try {
      ...
      let res = await Utility.cc98Fetch(
        `/topic/${this.props.voteInfo.topicId}/vote`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({
  ...
  getMessage() {
    switch (true) {
      case !Utility.isLogOn():
        return (
          <p>
            你还未登录，请先<Link to="/logOn">登陆</Link>。
          </p>
        );
      case this.props.voteInfo.canVote && this.props.voteInfo.needVote:
        return (
          <p>
            该投票在过期前，只有完成投票才能查看结果。你还未投过票，请先投票。
          </p>
        );
      case !!this.props.voteInfo.myRecord:
        return (
          <p>
            你已经投过票了，你的投票选项是：
            {this.props.voteInfo.myRecord.items.join("，")}。
          </p>
        );
      default:
        return null;
  ...
  render() {
    let shouldNotShowInfo =
      this.props.voteInfo.needVote &&
      !this.props.voteInfo.myRecord &&
      this.props.voteInfo.isAvailable;
    if (
      store.getState().userInfo.currentUserInfo.id ===
      this.props.topicInfo.userId
    )
      shouldNotShowInfo = false;
    return (
      <div className="vote-content">
        <div
          className="vote-message"
          style={{ opacity: this.state.messageOpacity }}
        >
          {this.state.message}
        </div>
            ...
            <p className="vote-description">
              <input
                disabled={
                  ...
                  (this.state.items.indexOf(item.id) === -1 &&
                    this.state.items.length ===
                      this.props.voteInfo.maxVoteCount) // 或者已经选中最多的选项
                }
              ...
              <span className="vote-total">
                <span
                  ...
                  style={{
                    ...
                    width: `${
                      (20 * item.count) /
                      (this.props.voteInfo.voteUserCount || 1)
                    }rem`,
                  }}
                ></span>
              </span>
              {shouldNotShowInfo ? null : (
                <span className="vote-item-info">
                  {item.count}人/
                  {(
                    (100 * item.count) /
                    (this.props.voteInfo.voteUserCount || 1)
        ...
        <div className="vote-info">
          <p>
            投票过期时间：
            {moment(this.props.voteInfo.expiredTime).format(
              "YYYY年MM月DD日 HH:mm:ss"
            )}
          </p>
          <p>
            每人最多允许投{this.props.voteInfo.maxVoteCount}票，已有
            {this.props.voteInfo.voteUserCount}人参与投票。
          </p>
          {this.getMessage()}
        </div>
        {!this.props.voteInfo.canVote ? null : (
          <div className="vote-buttons">
            <button
              className="button blue"
              type="button"
              onClick={this.submit}
              disabled={!this.state.items.length || this.state.isVoting}
            >
              投票
            </button>
            <button
