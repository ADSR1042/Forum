...
export class PostManagement extends React.Component<
  Props,
  {
    wealth: number;
    prestige: number;
...
> {
    ...
    switch (this.state.UI) {
      case "Award":
        if ($(id).val()) {
          ...
          if (this.state.wealth != 0) {
            status = await Utility.awardWealth(
              value,
              this.state.wealth,
              this.props.postId
            );
          ...
          if (this.state.prestige != 0) {
            status1 = await Utility.addPrestige(
              this.props.postId,
              this.state.prestige,
              value
            );
      ...
      case "Punish":
        if ($(id).val()) {
          ...
          if (this.state.wealth !== 0)
            status = await Utility.deductWealth(
              value,
              this.state.wealth,
              this.props.postId
            );
          ...
          if (this.state.prestige !== 0)
            status1 = await Utility.deductPrestige(
              this.props.postId,
              this.state.prestige,
              value
            );
          ...
          if (this.state.tpdays !== 0)
            status2 = await Utility.stopBoardPost(
              this.props.postId,
              value,
              this.state.tpdays
            );
      ...
      case "Delete":
        if (this.state.reason) {
          status = await Utility.deletePost(
            this.props.topicId,
            this.props.postId,
            this.state.reason
          );
        ...
        break;
      case "Other":
        await Utility.cancelStopBoardPost(
          this.props.userId,
          this.props.boardId
        );
        const UIId = `#manage${this.props.postId}`;
  ...
  render() {
    ...
    const awardUI = (
      <div className="column manageInfo" id="award">
        ...
        <div className="row manageOperation managePrestige">
          ...
          <input
            type="text"
            className="react-bootstrap-smalltext"
            value={this.state.prestige}
            onChange={this.prestigeInput}
          />
        </div>
        ...
        <div className="column manageOperation">
          ...
          >
            <option className="react-bootstrap-option" value="好文章">
              好文章
            </option>
            <option className="react-bootstrap-option" value="有用资源">
              有用资源
            </option>
            <option className="react-bootstrap-option" value="热心回复">
              热心回复
            </option>
    ...
    const punishUI = (
      <div className="column manageInfo" id="punish">
        <div className="row manageOperation">
        ...
        <div className="row manageOperation" style={{ display: "" }}>
          ...
          <input
            type="text"
            className="react-bootstrap-smalltext"
            value={this.state.tpdays}
            onChange={this.tpdaysInput}
          />
        </div>
        ...
        <div className="column manageOperation">
          ...
          >
            ...
            <select
              ...
              className="react-bootstrap-select"
            >
              <option className="react-bootstrap-option" value="人身攻击">
                人身攻击
              </option>
              <option className="react-bootstrap-option" value="违反版规">
                违反版规
              </option>
              <option className="react-bootstrap-option" value="恶意灌水">
                恶意灌水
              </option>
              <option className="react-bootstrap-option" value="自定义">
                自定义
    ...
    if (this.props.floor === 1)
      deleteAllow = <div>删除主题帖请在底部进行操作</div>;
    const deleteUI = (
      <div className="column manageInfo" id="punish">
        {deleteAllow}
      </div>
    ...
    const otherUI = (
      <div className="column manageInfo" id="other">
        <div>解除此用户版面tp</div>
      </div>
    );
    const UIId = `manage${this.props.postId}`;
    ...
    return (
      <div style={{ display: "none" }} id={UIId} className="postManagement">
        <div className="manageUI">
          <div className="row manageOptions">
            <div
              className="manageOptions-icon"
              id={awardOptionId}
              onClick={this.showAwardUI}
              style={{ color: "#FF7F00" }}
            >
              奖励
            </div>
        ...
        <div className="row" style={{ justifyContent: "space-around" }}>
          <button onClick={this.confirm} className="confirmManagement">
            确认
          </button>
          <button onClick={this.close} className="confirmManagement">
            关闭
          </button>
        </div>
      </div>
