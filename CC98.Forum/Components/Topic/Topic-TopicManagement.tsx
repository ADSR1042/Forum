...
export class TopicManagement extends React.Component<
  Props,
  {
    state;
    reason;
...
> {
  constructor(props) {
    ...
    this.state = {
      ...
      color: "#fff",
      topBoardName: "",
      childBoards: data.length > 0 ? data[0].boards : null,
    };
  }
  ...
  async confirm() {
    ...
    switch (this.state.state) {
        ...
        if (this.state.reason !== "") {
          switch ($("input[name='option']:checked").val()) {
            ...
            case "取消全站固顶":
              ...
              break;
            case "删除":
              status = await Utility.deleteTopic(
                this.props.topicInfo.id,
                this.state.reason
              );
              this.setState({ fetchState: status });
      ...
      case "highlight":
        const color = $("#custom")
          .spectrum("get")
          .toHexString($("#custom").spectrum("get"));
        if (this.state.reason !== "") {
  ...
  render() {
    ...
    let selectChildBoard = (
      ...
      >
        {this.state.childBoards.map(this.returnBoardOption)}
      </select>
    );
          ...
          <div className="row">
            <div className="row">
              <input
                type="radio"
                name="option"
                value={this.props.topicInfo.state === 1 ? "解锁" : "锁定"}
                onClick={
                  this.props.topicInfo.state === 1
                    ? this.showNormal
                    : this.showDays
                }
              />
          ...
          <div className="row" style={{ marginTop: "1rem" }}>
            <div className="row">
              <input
                ...
                onClick={
                  this.props.topicInfo.topState === 2
                    ? this.showNormal
                    : this.showDays
                }
              />
    ...
    return (
      <div style={{ display: "none" }} id={UIId} className="topicManagement">
        {UI}
        <div
          className="row"
          style={{ justifyContent: "space-around", marginTop: "4rem" }}
        >
