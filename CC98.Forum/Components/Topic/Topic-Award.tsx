...
> {
  ...
  async componentDidMount() {
    ...
    for (let i in award) {
      let thisUserInfo = Utility.getThisUserInfobyName(
        award[i].operatorName,
        usersInfo
      );
      //console.log("find");
  ...
  render() {
    ...
    const btn = (
      <button
        style={{ border: "#eaeaea solid thin", cursor: "pointer" }}
        onClick={this.state.showAll ? this.hideAll : this.showAll}
      >
        {this.state.showAll ? "收起" : "显示全部"}
      </button>
    );
    return (
      <div className="column awardInfo" id={awardInfoID}>
        <div
          className="row"
          style={{ width: "25rem", fontSize: "0.8rem", marginBottom: "0.2rem" }}
        >
