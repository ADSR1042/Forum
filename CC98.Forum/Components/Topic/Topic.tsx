...
> {
  ...
  async handleChange() {
    ...
    const newPage =
      (topicInfo.replyCount + 1) % 10 === 0
        ? (topicInfo.replyCount + 1) / 10
        : (topicInfo.replyCount + 1 - ((topicInfo.replyCount + 1) % 10)) / 10 +
          1;
    ...
    if (page != newPage) {
      ...
      if (
        (noticeSetting && noticeSetting.post === "是") ||
        (newPage == page + 1 && floor == 1)
      ) {
        console.log("要跳转");
        page = newPage;
        if (!page) page = 1;
        ...
        this.setState(
          { quote: { userName: "", content: "", replyTime: "", floor: "" } },
          this.props.history.push(url)
        );
      } else {
  ...
  render() {
    ...
    else sendTopic = <div>您还未登录，无法发言，请先登录。</div>;
    if (
    ...
    let topicHtml = (
      <div className="center">
        <DocumentTitle
          title={`${this.state.topicInfo.title || "帖子"} - CC98论坛`}
        />
