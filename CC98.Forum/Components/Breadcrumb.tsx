...
export class Breadcrumb extends React.Component<props> {
  render() {
    ...
    const elements = this.props.routers.map((item, index) =>
      item.path ? (
        <Link style={linkStyle(index)} key={item.breadcrumbName} to={item.path}>
          {item.breadcrumbName}
        </Link>
      ) : (
        <span style={linkStyle(index)} key={item.breadcrumbName}>
          {item.breadcrumbName}
        </span>
      )
    );
